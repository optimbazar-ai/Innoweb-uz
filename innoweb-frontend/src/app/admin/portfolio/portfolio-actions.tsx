...
export function PortfolioActions({ portfolioId }: { portfolioId: string }) {
  ...
  const handleDelete = async () => {
    const confirmDelete = window.confirm(
      "Haqiqatan ham ushbu portfolioni o'chirmoqchimisiz?"
    );
    ...
    try {
      ...
      if (response.ok) {
        window.alert("Portfolio muvaffaqiyatli o'chirildi!");
        router.refresh();
      } else {
        const data = await response.json().catch(() => ({}));
        window.alert(data?.error ?? "Portfolioni o'chirishda xatolik yuz berdi.");
      }
    } catch (error) {
  ...
  return (
    <div className="inline-flex gap-2">
      <Link 
        href={`/admin/portfolio/edit/${portfolioId}`}
