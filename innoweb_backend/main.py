...
app.add_middleware(
    ...
    allow_origins=[
        "http://localhost:3000",
        "http://127.0.0.1:3000",
        "https://innoweb-uz.vercel.app",  # Production frontend
    ],
...
@app.get("/posts/slug/{slug}", response_model=dict)
async def get_post_by_slug(slug: str):
    """Postni slug orqali olish (SEO-friendly URL)"""
    try:
        logger.info(f"'{slug}' slug'i orqali postni olish so'rovi")
        post = await db.post.find_unique(where={"slug": slug})
        if not post:
            logger.warning(f"'{slug}' slug'i bilan post topilmadi")
            raise HTTPException(status_code=404, detail="Post topilmadi")
        return {
            "success": True,
            "data": {
        ...
        logger.error(f"Postni olishda xatolik: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Xatolik: {str(e)}")
...
async def handle_contact_form(form_data: ContactForm):
    """Frontend'dan yuborilgan kontakt formasini qabul qiladi"""
    logger.info("Yangi kontakt formasi qabul qilindi")
    print("--- Yangi Xabar ---")
...
async def get_all_portfolio():
    """Barcha portfolio loyihalarini olish"""
    try:
        logger.info("Barcha portfolio loyihalarni olish so'rovi")
        ...
        logger.error(f"Portfolio olishda xatolik: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Xatolik: {str(e)}")


@app.get("/portfolio/{portfolio_id}", response_model=dict)
async def get_portfolio_by_id(portfolio_id: int):
    """Bitta portfolio loyihasini ID orqali olish"""
    try:
        logger.info(f"{portfolio_id} ID'li portfolio olish so'rovi")
        portfolio = await db.portfolio.find_unique(where={"id": portfolio_id})
        if not portfolio:
            logger.warning(f"{portfolio_id} ID'li portfolio topilmadi")
        ...
        return {
            ...
            "data": {
                ...
                "createdAt": str(portfolio.createdAt),
                "updatedAt": str(portfolio.updatedAt),
            }
        }
    except HTTPException:
        raise
...
@app.get("/portfolio/slug/{slug}", response_model=dict)
async def get_portfolio_by_slug(slug: str):
    """Portfolio loyihasini slug orqali olish (SEO-friendly URL)"""
    try:
        logger.info(f"'{slug}' slug'i orqali portfolio olish so'rovi")
        portfolio = await db.portfolio.find_unique(where={"slug": slug})
        if not portfolio:
            logger.warning(f"'{slug}' slug'i bilan portfolio topilmadi")
            raise HTTPException(status_code=404, detail="Portfolio topilmadi")
        return {
            "success": True,
            "data": {
...
async def create_portfolio(portfolio: PortfolioCreate, api_key: str = Depends(get_api_key)):
    """Yangi portfolio loyihasi yaratish"""
    try:
        logger.info(f"Yangi portfolio yaratish so'rovi: {portfolio.title}")
        ...
        return {
            ...
            "data": {
                ...
                "slug": new_portfolio.slug,
                "description": new_portfolio.description,
                "imageUrl": new_portfolio.imageUrl,
                "technologies": new_portfolio.technologies,
                "liveUrl": new_portfolio.liveUrl,
                "githubUrl": new_portfolio.githubUrl,
                "category": new_portfolio.category,
                "featured": new_portfolio.featured,
                "createdAt": str(new_portfolio.createdAt),
                "updatedAt": str(new_portfolio.updatedAt),
            }
        }
    except Exception as e:
        logger.error(f"Portfolio yaratishda xatolik: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Xatolik: {str(e)}")


@app.put("/portfolio/{portfolio_id}", response_model=dict)
async def update_portfolio(portfolio_id: int, portfolio_update: PortfolioUpdate, api_key: str = Depends(get_api_key)):
    """Mavjud portfolio loyihasini tahrirlash"""
    try:
        logger.info(f"{portfolio_id} ID'li portfolio yangilash so'rovi")
        existing_portfolio = await db.portfolio.find_unique(where={"id": portfolio_id})
        ...
        return {
            ...
            "data": {
                ...
                "slug": updated_portfolio.slug,
                "description": updated_portfolio.description,
                "imageUrl": updated_portfolio.imageUrl,
                "technologies": updated_portfolio.technologies,
                "liveUrl": updated_portfolio.liveUrl,
                "githubUrl": updated_portfolio.githubUrl,
                "category": updated_portfolio.category,
                "featured": updated_portfolio.featured,
                "createdAt": str(updated_portfolio.createdAt),
                "updatedAt": str(updated_portfolio.updatedAt),
            }
        }
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Portfolio yangilashda xatolik: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Xatolik: {str(e)}")


@app.delete("/portfolio/{portfolio_id}", response_model=dict)
async def delete_portfolio(portfolio_id: int, api_key: str = Depends(get_api_key)):
    """Portfolio loyihasini ID orqali o'chirish"""
    try:
        logger.info(f"{portfolio_id} ID'li portfolio o'chirish so'rovi")
        existing_portfolio = await db.portfolio.find_unique(where={"id": portfolio_id})
        if not existing_portfolio:
            raise HTTPException(status_code=404, detail="Portfolio topilmadi")

        await db.portfolio.delete(where={"id": portfolio_id})

        return {"success": True, "message": "Portfolio muvaffaqiyatli o'chirildi"}
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Portfolio o'chirishda xatolik: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Xatolik: {str(e)}")
